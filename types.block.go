...
type Block struct {
	...
	Paragraph        *ParagraphBlockData        `json:"paragraph,omitempty"`          // Paragraph block objects contain the following information within the paragraph property:
	...
	Callout          *CalloutBlockData          `json:"callout,omitempty"`            // Callout block objects contain the following information within the callout property:
	Quote            *QuoteBlockData            `json:"quote,omitempty"`              // Quote block objects contain the following information within the quote property
	BulletedListItem *BulletedListItemBlockData `json:"bulleted_list_item,omitempty"` // Bulleted list item block objects contain the following information within the bulleted_list_item property:
	NumberedListItem *NumberedListItemBlockData `json:"numbered_list_item,omitempty"` // Numbered list item block objects contain the following information within the numbered_list_item property:
	ToDo             *ToDoBlockData             `json:"to_do,omitempty"`              // To do block objects contain the following information within the to_do property:
	Toggle           *ToggleBlockData           `json:"toggle,omitempty"`             // Toggle block objects contain the following information within the toggle property:
	Code             *CodeBlockData             `json:"code,omitempty"`               // Code block objects contain the following information within the code property:
	ChildPage        *ChildPageBlockData        `json:"child_page,omitempty"`         // Child page block objects contain the following information within the child_page property: 📘Creating and Updating child_pagesTo create or update child_page type blocks, use the Create Page and the Update page endpoint.
	ChildDatabase    *ChildDatabaseBlockData    `json:"child_database,omitempty"`     // Child database block objects contain the following information within the child_database property: 📘Creating and Updating child_databasesTo create or update child_database type blocks, use the Create database and the Update database endpoint.
	...
	Image            *ImageBlockData            `json:"image,omitempty"`              // Includes supported image urls (i.e. ending in .png, .jpg, .jpeg, .gif, .tif, .tiff, .bmp, .svg, or .heic)
	Video            *VideoBlockData            `json:"video,omitempty"`              // Includes supported video urls (e.g. ending in .mkv, .flv, .gifv, .avi, .mov, .qt, .wmv, .asf, .amv, .mp4, .m4v, .mpeg, .mpv, .mpg, .f4v, etc.)
	File             *FileBlockData             `json:"file,omitempty"`
	Pdf              *PDFBlockData              `json:"pdf,omitempty"`
	Bookmark         *BookmarkBlockData         `json:"bookmark,omitempty"`          // Bookmark block objects contain the following information within the bookmark property:
	Equation         *EquationBlockData         `json:"equation,omitempty"`          // Equation block objects contain the following information within the equation property
	Divider          *DividerBlockData          `json:"divider,omitempty"`           // Divider block objects do not contain any information within the divider property
	TableOfContents  *TableOfContentsBlockData  `json:"table_of_contents,omitempty"` // Table of contents block objects contain the following information within the table_of_contents property
	Breadcrumb       *BreadcrumbBlockData       `json:"breadcrumb,omitempty"`        // Breadcrumb block objects do not contain any information within the breadcrumb  property
	...
	LinkPreview      *LinkPreviewBlockData      `json:"link_preview,omitempty"`      // Link Preview block objects return the originally pasted url. NOTE: The link_preview block will only be returned as part of a response. It cannot be created via the API.
	Template         *TemplateBlockData         `json:"template,omitempty"`          // Template block objects contain the following information within the template property:
	LinkToPage       *LinkToPageBlockData       `json:"link_to_page,omitempty"`      // Link to page objects contain a key corresponding with the value of type. The value is a type-specific string as described below.
...
type HeadingBlockData struct {
	...
	Color        string     `json:"color"`         // Color of the block. Possible values are: "default", "gray", "brown", "orange", "yellow", "green", "blue", "purple", "pink", "red", "gray_background", "brown_background", "orange_background", "yellow_background", "green_background", "blue_background", "purple_background", "pink_background", "red_background".
	IsToggleable bool       `json:"is_toggleable"` // undocumented
}
...
type BreadcrumbBlockData struct{}
